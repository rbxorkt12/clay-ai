...
from models.agents import Agent, AgentCreate, AgentUpdate, AgentResponse
...
async def get_agent(
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Agent not found: {str(e)}"
        )


@router.patch("/{agent_id}", response_model=AgentResponse)
async def update_agent(
    agent_id: str, agent_data: AgentUpdate, db: AsyncSession = Depends(get_db)
) -> AgentResponse:
    """Update agent by ID."""
    try:
...
async def delete_agent(
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Failed to delete agent: {str(e)}"
        )
