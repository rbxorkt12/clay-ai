...
linter_agent = LinterAgent(
    name="linter",
    config=AgentConfig(max_retries=3, timeout=300, memory_limit=512),
)


@router.post("/fix-bulk")
async def fix_errors_in_bulk(
    files: List[str], error_types: Optional[List[str]] = None
) -> BulkLinterResult:
    ...
    try:
        ...
        task = Task(
            type="linter", input={"files": files, "error_types": error_types}
        )
        ...
        raise HTTPException(
            status_code=500, detail=str(f"Error fixing linter errors: {e}")
        )
